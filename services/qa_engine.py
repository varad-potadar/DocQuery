...
def _generate(prompt: str) -> str:
    ...
    retries on 503/5xx, 429, and transient network errors with
    exponential backoff (1s, 2s, 4s, 8s across up to 5 attempts) before
    giving up."""
    client = _get_client()
...
def _clean_chunk(text: str) -> str:
    """
    Collapses runs of spaces/tabs (a common PDF/OCR extraction artifact)
    and reflows single mid-paragraph line breaks into spaces, but
    preserves blank-line boundaries between paragraphs/rows. The old
    version collapsed ALL whitespace runs -- including the blank lines
    csv_loader.py/xlsx_loader.py use to separate one row from the next --
    into a single space, so a multi-row chunk reached the LLM as one
    run-on line with no visible boundary between records at all. That's
    why a query for one ID could find the right chunk but the model
    couldn't reliably tell which Status/Date belonged to that ID versus
    the row next to it.
    """
    text = text.replace("\r\n", "\n").replace("\r", "\n")
    text = re.sub(r"[ \t]+", " ", text)
...
def answer_question(
    ...
    Returns:
        {
            ...
            "confidence":      str,         # "high" / "medium" / "low", or ""
                                             # if reranking failed and it
                                             # couldn't be computed -- see
                                             # _confidence_label(); heuristic,
    ...
    #
    #     Best-effort: reranking/confidence must never be able to block
    #     an answer from being returned. If the cross-encoder fails to
    #     load, runs out of memory, or throws for any other reason, we
    #     fall back to the unreranked retrieval order and skip the
    #     confidence label rather than raising -- a correct answer with
    #     no confidence tag beats no answer at all.
    try:
        ...
        print(f"[qa_engine] Reranking failed, continuing without it: {e}")
        reranked = retrieved
        confidence = ""
...
Answer:"""
    ...
    # 8. LLM call. See _generate()'s retry_with_backoff wrapping above --
    #    if every attempt fails, or the error isn't retriable at all
    #    (e.g. a bad API key), the exception lands here and we still
    #    return a real response instead of crashing the caller.
    try:

...
Each stored item:
    {
        "text":       str,   # clean chunk text -- shown to the LLM, cited
        "embed_text": str,   # what's actually embedded + BM25-indexed;
                              # usually "doc title — heading — text" so a
                              # chunk is findable by words that only
                              # appear in its title/heading, not its body
                              # (see ingest.py). Falls back to "text" when
                              # a caller doesn't provide it.
        "doc_id":     str,
        "chunk_index": int,
        "char_start": int,
        "page":       Optional[int],
...
class VectorStore:
    ...
    def add(self, embeddings: np.ndarray, chunks: List[Dict], doc_id: str):
        ...
        chunks: list of dicts from chunker.chunk_text() or chunk_sections()
            ...
            "embed_text", when present (see ingest.py), is what gets
            embedded/BM25-indexed instead of "text" -- falls back to
            "text" so older callers still work unchanged.
        """
        ...
        for chunk in chunks:
            self.data.append({
                "text":        chunk["text"],
                "embed_text":  chunk.get("embed_text", chunk["text"]),
                "doc_id":      doc_id,
    ...
    def _rebuild_bm25(self):
        if not HAS_BM25 or not self.data:
            self._bm25 = None
            return
        corpus = [_tokenize(item["embed_text"]) for item in self.data]
